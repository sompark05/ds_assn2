...
/*
    ...
    a.	Implement BinaryTree::findHeightOfNode, BinaryTree::findLevelOfNode and
   BinaryTree::levelOrder function that can traverse a binary tree with given
   level order traverse mode and finds the level and height of a specific node.
    You can additionally implement other functions to facilitate the traversal
   of a binary tree (HINT : BinaryTree::_currentLevel used in level order
   traverse mode.)
    ** We only consider 1 to 9 integer inputs for binary tree nodes.
    ...
    Input:
    -	String with bracket representation and String representing specific
   node.

    Output:
    -	A sequence of node values acquired from the ① level order traverse mode
   and the ② height and ③ level of a specific node. The value is separated with
   a white space for traverse mode, and height level with enter. Check the
   example below. -	You should print out -1 for each height and level for
   nodes that are not in the binary tree.
...
/*
    ...
    BinaryTree::leafCount calculates the number of leaves of the given binary
   tree. BinaryTree::getWidth calculates width, the largest number of nodes at
   any single level. Maximum depth of the tree is limited to 100.
...
void task_4(ofstream &fout, const char *tree_string, const char *instr_seq) {
  ...
  try {
    ...
    if (strcmp(instr_seq, "leaf") == 0) {
      fout << tree.leafCount(tree.getRoot()) << endl;
